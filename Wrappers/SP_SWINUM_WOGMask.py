...
class SP_SWINUM_WOGMASK_Wrapper(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        samples = []
        for masked, labels in zip(pred_numpy, segments.cpu().numpy()):
            plt_image = masked[labels-1].reshape([mask.size(-2), mask.size(-1)])
            samples.append(plt_image)

        samples = torch.tensor(np.expand_dims(np.array(samples), 1)).cuda()

        # samples = F.interpolate(samples, (mask.size(-2), mask.size(-1)))
