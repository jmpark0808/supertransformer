...
import torch
from Models.SP_GAT_PyG import SP_GAT_IN
...
class SP_ImageNet_GAT_PyG_Wrapper(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        self.supert = SP_GAT_IN(input_dim, self.tfm_hp[1], self.dropout, self.dropout_edge, self.tfm_hp[0], self.tfm_hp[2], self.num_seg)
        if self.load:
    ...
    def training_step(self, batch, batch_idx):
        ...
        features = sample.x
        features = features.reshape(target.size(0), 32, 32, -1).permute(0, 3, 1, 2)
        features, target = self.mixup(features, target)
        features = features.permute(0, 2, 3, 1).reshape(target.size(0)*1024, -1)
        sample.x = features
