...
class SP_SWINUM_DA_Wrapper(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        self.ema_alpha = kwargs.get('ema_alpha')
        ...
        self.supert_student = SwinUTransformer(img_size=res, in_chans=input_dim, patch_size=1, window_size=self.window_size,
        ...
        self.supert_teacher = SwinUTransformer(img_size=res, in_chans=input_dim, patch_size=1, window_size=self.window_size,
                                       embed_dim=self.dims, depths=self.depths,
                                         num_heads=self.heads, mlp_ratio=self.mlp_ratio, attn_drop_rate=self.dropout_edge, drop_rate=self.dropout,
                                         drop_path_rate=self.dp)
    ...
    def configure_optimizers(self):
        ...
        for name, param in self.supert_student.named_parameters():
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch < 5:
            ...
            features_source = source_batch['features']
            seq_mask = source_batch['seq_mask']
            segments = source_batch['segments']
            mask = source_batch['mask']
            ...
            if self.aug_strat == 4 and 'RS' not in self.dataloader:
                ...
                features_source, seq_mask = self.mixup(features_source, seq_mask)
            ...
            pred = self.forward(features_source)
            
            loss_src = self.loss(pred, seq_mask)
            ...
            self.num_samples += features_source.size(0)
            ...
            update_ema(self.supert_student, self.supert_teacher, self.ema_alpha)
        return loss
    ...
    def on_validation_epoch_end(self):
        ...
        self.log('Validation Max F Score', torch.max(f_score), sync_dist=True)
        # self.log('Validation Max F Threshold', thlist[torch.argmax(f_score)])

        self.log('Validation MAE', self.maes/self.mean_num, sync_dist=True)
        ...
        f_score = (1 + beta_square) * prec * recall / (beta_square * prec + recall)
        thlist = torch.linspace(0, 1 - 1e-10, self.num_thresholds)
        self.log('DUTS Max F Score', torch.max(f_score), sync_dist=True)
        # self.log('Test Max F Threshold', thlist[torch.argmax(f_score)])

        self.log('DUTS MAE', self.duts_maes_test/self.duts_mean_num_test, sync_dist=True)
        ...
        f_score = (1 + beta_square) * prec * recall / (beta_square * prec + recall)
        thlist = torch.linspace(0, 1 - 1e-10, self.num_thresholds)
        self.log('DUTSO Max F Score', torch.max(f_score), sync_dist=True)
        # self.log('Test Max F Threshold', thlist[torch.argmax(f_score)])

        self.log('DUTSO MAE', self.dutso_maes_test/self.dutso_mean_num_test, sync_dist=True)
        ...
        f_score = (1 + beta_square) * prec * recall / (beta_square * prec + recall)
        thlist = torch.linspace(0, 1 - 1e-10, self.num_thresholds)
        self.log('ECSSD Max F Score', torch.max(f_score), sync_dist=True)
        # self.log('Test Max F Threshold', thlist[torch.argmax(f_score)])

        self.log('ECSSD MAE', self.ecssd_maes_test/self.ecssd_mean_num_test, sync_dist=True)
        ...
        f_score = (1 + beta_square) * prec * recall / (beta_square * prec + recall)
        thlist = torch.linspace(0, 1 - 1e-10, self.num_thresholds)
        self.log('HKU Max F Score', torch.max(f_score), sync_dist=True)
        # self.log('Test Max F Threshold', thlist[torch.argmax(f_score)])

        self.log('HKU MAE', self.hku_maes_test/self.hku_mean_num_test, sync_dist=True)
        ...
        f_score = (1 + beta_square) * prec * recall / (beta_square * prec + recall)
        thlist = torch.linspace(0, 1 - 1e-10, self.num_thresholds)
        self.log('PASCAL Max F Score', torch.max(f_score), sync_dist=True)
        # self.log('Test Max F Threshold', thlist[torch.argmax(f_score)])

        self.log('PASCAL MAE', self.pascal_maes_test/self.pascal_mean_num_test, sync_dist=True)
        # if self.current_epoch >= self.warmup_epochs:
    ...
    def on_test_epoch_end(self):
        ...
        f_score = (1 + beta_square) * prec * recall / (beta_square * prec + recall)
        thlist = torch.linspace(0, 1 - 1e-10, 256)
        self.log('Final Test Max F Score', torch.max(f_score), sync_dist=True)

        self.log('Final Test MAE', self.maes/self.mean_num, sync_dist=True)
        self.log('Final Test E measure', torch.max(self.e_measure_scores)/self.mean_num, sync_dist=True)
        self.log('Final Test S measure', self.s_measure_q/self.mean_num, sync_dist=True)
