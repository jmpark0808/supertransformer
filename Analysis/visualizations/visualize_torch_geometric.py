...
class GDPModel(torch.nn.Module):
    def __init__(self, num_features=3, hidden_size=32, target_size=1):
        ...
        self.convs = [GATv2Conv(self.num_features, self.hidden_size, edge_dim = 2),
                      GATv2Conv(self.hidden_size, self.hidden_size, edge_dim = 2)]
...
class SPDataset(datatorch.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        edge_features = torch.ones(random_int, random_int, 2)
        edge_features = edge_features[adj.nonzero().t().numpy()]


        return Data(x=t, edge_index=edge_index, edge_attr=edge_features)
...
model = GDPModel(3, 32, 1)
from tqdm import tqdm
from torchviz import make_dot
for d in loader:
    yhat = model(d)
    make_dot(yhat, params=dict(list(model.named_parameters()))).render('gnn_graph', format='png')
    # print(out.size())
    break