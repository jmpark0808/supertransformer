...
class ViPEnc(nn.Module):
    def __init__(self, *, image_size, patch_size, dims, depths, heads, mlp_ratio, 
        ...
        #                             heads[idx], dims[idx]//heads[idx], int(mlp_ratio*dims[idx]),emb_dropout, dropout, downsample=True))
        for idx, depth in enumerate(depths):
            assert dims[idx]%dims[0] == 0
            num_tokens = dims[idx]//dims[0]
            if idx == len(depths)-1:
                self.transformer_enc.append(TransformerEncoder(dims[idx], dims[idx], None, depth,
                                    heads[idx], dims[idx]//heads[idx], int(mlp_ratio*dims[idx]),emb_dropout, dropout, downsample=False, num_tokens=num_tokens))
            else:
                self.transformer_enc.append(TransformerEncoder(dims[idx], dims[idx+1], None, depth,
                                    heads[idx], dims[idx]//heads[idx], int(mlp_ratio*dims[idx]),emb_dropout, dropout, downsample=False, num_tokens=num_tokens))
