...
def kernel_partition(x, window_size, unfold):
    ...
    B, H, W, C = x.shape
    x = x.permute(0, 3, 1, 2) # B C H W
    x = unfold(x) # B C*w*w H*W
    x = x.reshape(B, C, window_size, window_size, -1) # B, C, w, w, H*W
    windows = x.permute(0, 4, 2, 3, 1).contiguous().view(-1, window_size, window_size, C) # B*H*W, w, w, C
    return windows
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, pos, mask=None):
        ...
        rpe_k = rpe @ self.pos_linear_k # B, N, N, D
        
        rpe_v = rpe @ self.pos_linear_v # B, N, N, D
...
class TopKAttention(nn.Module):
    ...
    def forward(self, x, windowed_x, mask=None):
        """
        Args:
            x: input features with shape of (B, n*num_windows, C)
            windowed_x: input features with shape of (num_windows*B, n, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
        """
        B_, n, C = windowed_x.shape
        B, N, C = x.shape
        kv = self.kv(windowed_x).reshape(B_, n, 2, self.num_heads, C).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]  # make torchscript happy (cannot use tensor as tuple) # num_windows*B, H, N, C
        score = (k*self.weight).sum(dim=-1) # num_windows*B, H, N
        score = self.act(score / self.weight.norm(p=2, dim=-1)) # num_windows*B, H, N
        argmax = torch.argmax(score, dim=-1, keepdim=True) # num_windows*B, H
        k = torch.gather(k, 2, argmax.unsqueeze(-1).repeat(1, 1, 1, C)) # num_windows*B, H, C
        k = k.reshape(-1, B, self.num_heads, C).permute(1, 2, 0, 3) # B, H, num_windows, C
        v = torch.gather(v, 2, argmax.unsqueeze(-1).repeat(1, 1, 1, C)) # num_windows*B, H, C
        v = v.reshape(-1, B, self.num_heads, C).permute(1, 2, 0, 3) # B, H, num_windows, C

        q = self.q(x).reshape(B, N, self.num_heads, C).permute(0, 2, 1, 3) # B, H, N, C

        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))# B, H, N, num_windows
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, self.num_heads, C) # B, N, H, C
        x = torch.mean(x, dim=2)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class KernelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1)) # B_, num_heads, 1, N
...
class KernelTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, head_dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.unfold = nn.Unfold(window_size, shift_size, (window_size//2)*(shift_size))
    ...
    def forward(self, x):
        ...
        # partition windows
        x_windows = kernel_partition(x, self.window_size, self.unfold)  # B*H*W, window_size, window_size, C
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # B*H*W, window_size*window_size, C

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows)  # B*H*W, 1, C

        # merge windows
        x = attn_windows.reshape(B, H, W, C)
        x = x.view(B, H * W, C)
...
class PatchUnMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.enlarge = nn.Linear(dim, 2 * dim, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.enlarge(x) # B H//2 W//2 2*C

        x = x.reshape(B, H, W, C//2)

        x = x.view(B, -1, C//2)  # B H*2*W*2 C/2
...
class BasicLayerKernel(nn.Module):
    """ A basic Swin Transformer layer for one stage.
...
class SwinKernelTransformer(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, options, **kwargs):
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = BasicLayerKernel(dim=embed_dim, 
                               input_resolution=(patches_resolution[0],
                                                 patches_resolution[1]),
                               depth=depths[i_layer],
                               num_heads=num_heads[i_layer],
...
if __name__ == '__main__':
    ...
    options = {'swin_hp': {'patch_size': 1,  # (int | tuple(int)): Patch size. Default: 4
        ...
        'num_heads': [4, 8, 8, 16], #(tuple(int)): Number of attention heads in different layers.
    ...
    pos = torch.rand((1, 1024, 16))
    input = torch.rand((1, 36, 32, 32))

    output = model(input, pos)
