...
class SwinUTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, fused_window_process=False, mode=1, **kwargs):
        super().__init__()
        if mode == 1: # Just Colour
            in_chans = 6
        elif mode == 2: # Colour + learnable PE
            
        elif mode == 3: # Colour + RoPE

        elif mode == 4: # Colour + Centroid PE
            pass
            # Do PE stuff here
        elif mode == 5: # Colour + Centroid PE + FFT
        else:
            assert 0, 'Mode is not recognized'
        self.mode = mode
        ...
        if mode == 1: # Just Colour
            ...
            self.locations = nn.Parameter(torch.randn(1, img_size*img_size, embed_dim[0]))
        elif mode == 3: # Colour + RoPE
            self.locations = None
        elif mode == 4: # Colour + Centroid PE
            pass
    ...
    def forward(self, x):
        ...
        lbp = x[:, -10:, :, :]
        color = x[:, 2:8, :, :]
        if self.mode == 1: # Just colour
            x = color
        elif self.mode == 2: # Colour + learnable PE
            x = color
            locations = self.locations
        elif self.mode == 3: # Colour + RoPE
            x = color
            locations = self.locations
        elif self.mode == 4: # Colour + Centroid PE
            x = color
        ...
        elif self.mode == 5: # Colour + Centroid PE + FFT
            assert 0, 'This is already done'
