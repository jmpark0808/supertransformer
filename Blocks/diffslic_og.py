...
if __name__=='__main__':
    ...
    with torch.no_grad():
        ...
        diff_slic = DiffSLIC(args.n_spix, args.n_iter, args.tau, args.candidate_radius, stable=args.stable, normalize=False)
        st_time = time.time()
        ...
        label_onehot = F.one_hot(label.long().reshape(1, -1), args.n_spix).float()
        ...
        Bs = ((img+1)/2.).reshape(1, 3, -1).permute(0, 2, 1)
    ...
    ax1.imshow(mark_boundaries(colour.detach().numpy()[0, np_lbl.reshape(-1), :3].reshape(768, 1024, 3), np_lbl))
    ax1.scatter(centroids[0, :, 0], centroids[0, :, 1], s=10, c='red')
