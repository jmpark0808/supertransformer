...
class YDDataModule(pl.LightningDataModule):

    def __init__(self, **kwargs):
        super().__init__()

        self.root_dir = kwargs.get('dataset_tr')
        ...
        for root, subdirs, files in os.walk(os.path.join(davis_train_dir, 'Annotations')):
            for file in files:
                ...
                self.train_mask_list.append(os.path.join(davis_train_dir, 'Annotations', tag_mask))
                self.train_image_list.append(os.path.join(davis_train_dir, 'JPEGImages', tag_image))

        for root, subdirs, files in os.walk(os.path.join(youtube_vos_train_dir, 'Annotations')):
            for file in files:
                tag_mask = os.path.join(root.split('/')[-1], file)
                tag_image = tag_mask.replace('png', 'jpg')
                self.train_mask_list.append(os.path.join(youtube_vos_train_dir, 'Annotations', tag_mask))
                self.train_image_list.append(os.path.join(youtube_vos_train_dir, 'JPEGImages',  tag_image))


        for root, subdirs, files in os.walk(os.path.join(youtube_vos_valid_dir, 'Annotations')):
            for file in files:
                tag_mask = os.path.join(root.split('/')[-1], file)
                tag_image = tag_mask.replace('png', 'jpg')
                self.valid_mask_list.append(os.path.join(youtube_vos_valid_dir, 'Annotations', tag_mask))
                self.valid_image_list.append(os.path.join(youtube_vos_valid_dir, 'JPEGImages',  tag_image))

        for root, subdirs, files in os.walk(os.path.join(youtube_vos_test_dir, 'Annotations')):
            for file in files:
                tag_mask = os.path.join(root.split('/')[-1], file)
                tag_image = tag_mask.replace('png', 'jpg')
                self.test_mask_list.append(os.path.join(youtube_vos_test_dir, 'Annotations', tag_mask))
                self.test_image_list.append(os.path.join(youtube_vos_test_dir, 'JPEGImages',  tag_image))

        
        if self.debug:
            self.valid_image_list = self.valid_image_list[:100]
            self.valid_mask_list = self.valid_mask_list[:100]

            self.train_image_list = self.train_image_list[:100]
            self.train_mask_list = self.train_mask_list[:100]
