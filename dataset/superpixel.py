...
class SPFFTDataset(data.Dataset):
    def __init__(self, root_dir, num_seg, size, data_augmentation=True):
        self.root_dir = root_dir
        self.image_list = sorted(os.listdir('{}/Image'.format(root_dir)))
        self.mask_list = sorted(os.listdir('{}/Mask'.format(root_dir)))
        self.transform = transforms.Compose(
            [RandomFlip(0.5),
             RandomCrop(size, int(size*1.14)),
             ToTensorSPFFT(num_seg)])
        if not data_augmentation:
            self.transform = transforms.Compose([Resize(size), ToTensorSPFFT(num_seg)])
...
class SPFFTDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        data_train = SPFFTDataset(self.train_dir, self.num_seg, self.res, True)
        return DataLoader(
                data_train, batch_size=self.batch_size, 
                num_workers=self.num_workers, shuffle=True, pin_memory=False)

    def val_dataloader(self):
        data_val = SPFFTDataset(self.val_dir, self.num_seg, self.res, False)
        return DataLoader(
                data_val, batch_size=self.batch_size, 
                num_workers=self.num_workers, pin_memory=False)

    def test_dataloader(self):
        data_test = SPFFTDataset(self.test_dir, self.num_seg, self.res,  False)
        return DataLoader(
                data_test, batch_size=self.batch_size, 
                num_workers=self.num_workers, pin_memory=False)
