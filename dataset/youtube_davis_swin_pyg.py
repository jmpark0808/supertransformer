...
class SPDatasetExport(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        np.save(sp_file_path_edge_attr, spatial_distances)
        # vs_right = np.vstack([segments[:,:-1].ravel(), segments[:,1:].ravel()])
        # vs_below = np.vstack([segments[:-1,:].ravel(), segments[1:,:].ravel()])
        # vs_diagonal_r = np.vstack([segments[:-1,:-1].ravel(), segments[1:,1:].ravel()])
        # vs_diagonal_l = np.vstack([segments[1:,:-1].ravel(), segments[:-1,1:].ravel()])
...
class SPDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        sp_file_name_edge_attr = "".join(self.image_list[item].split('/')[-2:]).split('.')[0]+'_edge_attr.npy'
        ...
        edge_attr = np.load(sp_file_path_edge_attr)
...
class YDGDataModule(pl.LightningDataModule):
