...
class ImageNetDataset(data.Dataset):
    def __init__(self, root_dir, augmentation, coeff, size):
        ...
        local_unfold = nn.Unfold(3, 1, (3//2))
        
        local_index = local_unfold(node_index)
        all_local_indices = []

        for i in range(local_index.shape[1]):
            x, y = torch.meshgrid(local_index[3**2//2, i], local_index[:, i])
        ...
        all_local_indices = torch.cat(all_local_indices, dim=1).long()
    
        
        self.edge_indices = all_local_indices
    ...
    def __getitem__(self, item):
        ...
        xs = features[:, 0]
        ys = features[:, 1]
        rel_xs = xs[self.edge_indices[0, :]] - xs[self.edge_indices[1, :]]
        rel_ys = ys[self.edge_indices[0, :]] - ys[self.edge_indices[1, :]]
        edge_attr = torch.stack((rel_xs, rel_ys), dim=1)
        sample = Data(x=features,
                        edge_index=self.edge_indices, edge_attr=edge_attr)
...
class SPGSImageNetDataModule(pl.LightningDataModule):
