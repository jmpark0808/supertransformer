...
class ToTensorSPFFT(object):
    ...
    def __call__(self, sample):
        ...
        flat_seg = mask_np.ravel()
        flat_seg[flat_seg == 255] = 182
        flat_spx = segments.ravel()-1 
        
        # Get number of classes
        N = 183
        votes = np.zeros((self.num_seg, N), dtype=np.int32)
        
        np.add.at(votes, (flat_spx, flat_seg), 1)
        seq_mask = votes.argmax(axis=1)
...
class SPDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        mask = mask.convert('L').resize((self.size, self.size), resample=Image.NEAREST)
...
class SPSemSegDataModule(pl.LightningDataModule):
