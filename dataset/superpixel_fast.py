...
class ToTensorSPRSFFT(object):
    ...
    def __call__(self, sample):
        ...
        img_np = np.array(img)
        shape_cache = img_np.shape
        res = self.num_seg**0.5
        rounded_H = int(math.ceil(img_np.shape[0]/res)*res)
        rounded_W = int(math.ceil(img_np.shape[1]/res)*res)
        max_dim = np.max([rounded_H, rounded_W])
        img = img.resize((max_dim, max_dim))
        mask = mask.resize((max_dim, max_dim))
        img_np = np.array(img)
        img_gray = np.array(img.convert('L'))
        ...
        return features, seq_mask, segments, mask, img_np, edge_attr
...
class SPDatasetExport(data.Dataset):
    def __init__(self, image_list, mask_list, num_seg, size, compactness,
        ...
        if dataloader == 'SPFFFT':
            ...
            self.transform = transforms.Compose([Resize(size),
             totensor])
        elif dataloader == 'SPFRS':
            totensor = ToTensorSPRSFFT(num_seg, compactness, coeff, size, ignore_phase)
    ...
    def __getitem__(self, item):
        ...
        if self.dataloader == 'SPFRS':
            np.save(sp_file_path_mask, sample[3])
        else:
...
class SPDatasetWOGMask(data.Dataset):
    def __init__(self, image_list, mask_list, num_seg, size, 
                  dataloader, data_augmentation=True, coeff=None):
        self.image_list = image_list
        self.mask_list = mask_list
...
class SPFRSDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        data_train = SPDatasetWOGMask(self.tr_image_list, self.tr_mask_list, self.num_seg,
                                self.res, self.dataloader, True,
                                  self.coeff)
        return DataLoader(
                data_train, batch_size=self.batch_size, 
    ...
    def val_dataloader(self):
        data_test = SPDatasetWOGMask(self.test_image_list, self.test_mask_list, self.num_seg,
                               self.res,  self.dataloader, False, 
                               self.coeff)
